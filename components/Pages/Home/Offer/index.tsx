    ...
    const data = [
        {
            ...
            image: 'przeciwbolowy-plecow.webp'
        },
        {
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-4 md:mt-8">
            {
                data.map((item, index) => (
                    <div key={index} className="py-4 bg-white text-center p-2 border border-gray-900 relative h-[220px]">
                        ...
                        <Image src={`/images/offer/massages/${item.image}`} alt={item.title} layout="fill" objectFit="cover" className="absolute inset-0 w-full h-full object-cover z-0" />
                    </div>
                ))
            }
        </section>
